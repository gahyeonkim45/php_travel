...
if($page != 1) { 
	$paging .= '<li class="page page_start"><a href="AfterConcert.php?page=1' . $subString . '">처음</a></li>'; 
} 
//첫 섹션이 아니라면 이전 버튼을 생성 
if($currentSection != 1) { 
	$paging .= '<li class="page page_prev"><a href="AfterConcert.php?page=' . $prevPage . $subString . '">이전</a></li>'; 
} 
...
for($i = $firstPage; $i <= $lastPage; $i++) { 
	if($i == $page) { 
		...
		$paging .= '<li class="page"><a href="AfterConcert.php?page=' . $i . $subString . '">' . $i . '</a></li>'; 
	} 
} 

//마지막 섹션이 아니라면 다음 버튼을 생성 
if($currentSection != $allSection) { 
	$paging .= '<li class="page page_next"><a href="AfterConcert.php?page=' . $nextPage . $subString . '">다음</a></li>'; 
} 

//마지막 페이지가 아니라면 끝 버튼을 생성 
if($page != $allPage) { 
	$paging .= '<li class="page page_end"><a href="AfterConcert.php?page=' . $allPage . $subString . '">끝</a></li>'; 
} 
$paging .= '</ul>'; 
...
<body> 
	<div id="all"> 
		...
		<div class="btnSet" style="position: absolute;right: 7px;bottom: -16px;"> 
			<button><a href="write.php" class="btnWrite_btn" style="text-decoration:none;">글쓰기</a> </button>
		</div> 
		...
		<div class="searchBox" style="position: absolute;right: 67px;bottom: -16px;"> 
			<form action="AfterConcert.php" method="get"> 
