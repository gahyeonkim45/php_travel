...
$bno = $_GET['bno'];
if(!empty($bNo) && empty($_COOKIE['board_trip_' . $bNo])) {
	$sql = 'update board_trip set b_hit = b_hit + 1 where b_no = ' . $bNo;
	$result = $db->query($sql); 
	if(empty($result)) {
		?>
		<script>
		...
		setcookie('board_trip' . $bNo, TRUE, time() + (60 * 60 * 24), '/');
	}
}
$sql = 'select b_title, b_content, b_date, b_hit, b_id from board_trip where b_no = ' . $bno;
$result = $db->query($sql);
...
<body>
    ...
    <article class="boardArticle2">
     ...
     <div class="btnSet">
       <a href="trip_write.php?bno=<?php echo $bno?>">수정</a>
       <a href="AfterTrip.php">목록</a>
     </div>
